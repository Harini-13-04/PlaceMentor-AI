...
const SAMPLE_CHALLENGE_QUESTIONS = [
  {
    ...
    difficulty: "Medium",
    question: "If a company's revenue increased by 30% and expenses decreased by 10%, what happened to the profit margin if original revenue was ₹100 and expenses were ₹60?",
    options: ["Increased from 40% to 58.4%", "Increased from 40% to 55%", "Decreased by 5%", "Remained unchanged"],
    correctIndex: 0,
    explanation: "Original profit = 100 - 60 = 40 (40%). New revenue = 130, new expenses = 54. New profit = 130 - 54 = 76. Profit margin = 76 / 130 = 58.46%.",
  },
  {
    id: "dc-2",
    category: "Data Interpretation",
    difficulty: "Easy",
    question: "In a pie chart with total expenditure ₹2,40,000, Marketing occupies 54°. How much is spent on Marketing?",
    options: ["₹28,000", "₹36,000", "₹42,000", "₹48,000"],
    correctIndex: 1,
    explanation: "Fraction = 54° / 360° = 3/20 = 15%. Expenditure = 15% of 2,40,000 = ₹36,000.",
  },
  {
    id: "dc-3",
    category: "Logical Reasoning",
    difficulty: "Medium",
    question: "Find the odd one out in the number series: 3, 5, 9, 11, 15, 17, 21, 24",
    options: ["15", "17", "21", "24"],
    correctIndex: 3,
    explanation: "Alternating differences are +2, +4, +2, +4, +2, +4... 21 + 4 should be 25, not 24.",
  },
  {
    id: "dc-4",
    category: "Core CS",
    difficulty: "Easy",
    question: "Which data structure provides O(1) average lookup and insertion time?",
    options: ["Hash Table", "Binary Search Tree", "Linked List", "Array List"],
    correctIndex: 0,
    explanation: "Hash Tables compute bucket indices in O(1) average time via hash functions.",
  },
  {
    id: "dc-5",
    category: "DBMS",
    difficulty: "Easy",
    question: "What is the primary objective of a database foreign key constraint?",
    options: ["Referential Integrity", "Faster Disk Writes", "Table Compression", "Automatic Index Deletion"],
    correctIndex: 0,
    explanation: "Foreign keys enforce Referential Integrity between related parent and child tables.",
  },
];

export default function DailyChallengePanel({ className = "" }: DailyChallengePanelProps) {
  ...
  // =========================================================================
  // FULL-PAGE DEDICATED CHALLENGE WORKSPACE (Viewport Filling Mode)
  // =========================================================================
  if (isFullPageMode) {
    return (
      <div className="fixed inset-0 z-50 bg-background text-foreground flex flex-col p-4 sm:p-8 overflow-y-auto font-sans">
        <div className="w-full max-w-4xl mx-auto space-y-6 flex-1 flex flex-col justify-between">
          ...
          {!isSubmitted ? (
            <div className="p-6 sm:p-8 rounded-2xl border border-border bg-card shadow-sm space-y-6 flex-1 flex flex-col justify-between my-2">
              ...
              <div className="flex items-center justify-between pt-6 border-t border-border">
                ...
                {currentQuestionIdx < SAMPLE_CHALLENGE_QUESTIONS.length - 1 ? (
                  <button
                    onClick={() => setCurrentQuestionIdx((i) => i + 1)}
                    className="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold shadow-sm"
                  >
                    Next Question
                  </button>
                ) : (
                  <button
                    ...
                    className="px-7 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:opacity-95 text-white text-xs font-bold shadow-md"
                  >
                    Submit Challenge
                  </button>
                )}
            ...
            /* Post Submission Full-Page Diagnostic View */
            <div className="space-y-6 my-2">
              <div className="p-8 rounded-2xl border border-purple-500/25 bg-gradient-to-br from-purple-500/10 via-card to-card dark:from-purple-950/40 dark:via-secondary dark:to-card text-center space-y-3 shadow-sm">
                ...
                <p className="text-sm text-muted-foreground">
                  You scored <span className="text-purple-600 dark:text-purple-400 font-bold">{userScore.solved}/{userScore.total}</span> ({userScore.accuracy}% Accuracy) in {userScore.time}.
                </p>
                ...
                <div className="grid grid-cols-3 gap-3 max-w-md mx-auto pt-3">
                  <div className="p-3.5 rounded-xl bg-card border border-border">
                    ...
                    <p className="text-lg font-mono font-bold text-purple-600 dark:text-purple-400">{userScore.solved}/{userScore.total}</p>
                  </div>
                  <div className="p-3.5 rounded-xl bg-card border border-border">
                    <p className="text-[10px] text-muted-foreground font-semibold">Accuracy</p>
                    <p className="text-lg font-mono font-bold text-teal-600 dark:text-teal-400">{userScore.accuracy}%</p>
                  </div>
                  <div className="p-3.5 rounded-xl bg-card border border-border">
                    <p className="text-[10px] text-muted-foreground font-semibold">Time</p>
                    <p className="text-lg font-mono font-bold text-amber-600 dark:text-amber-400">{userScore.time}</p>
                  </div>
                </div>
              </div>
  ...
  // =========================================================================
  // STANDARD SIDEBAR / DASHBOARD WIDGET VIEW
  // =========================================================================
  return (
    <aside className={`w-full lg:w-[320px] shrink-0 space-y-4 font-sans select-none ${className}`}>
      {/* 1. Main Daily Challenge Card */}
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3.5 shadow-sm relative overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-6 h-6 rounded-lg bg-rose-500/15 border border-rose-500/30 flex items-center justify-center text-rose-500 text-xs">
              🎯
            </span>
        ...
        {/* Challenge Box */}
        <div className="p-3.5 rounded-xl border border-purple-500/25 bg-gradient-to-br from-purple-500/10 via-card to-card dark:from-purple-950/40 dark:via-secondary dark:to-card space-y-2.5">
          <div className="flex items-center justify-between">
          ...
          <div className="pt-1 text-[11px] text-muted-foreground leading-snug">
            <p className="font-semibold text-purple-700 dark:text-purple-300 text-[10px] uppercase tracking-wider">Recommended for you</p>
            <p className="text-muted-foreground mt-0.5">Your DI accuracy dropped by 12% this week.</p>
          </div>
        ...
        {/* 2. Your Performance Box */}
        <div className="p-3 rounded-xl border border-border bg-secondary/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Your Performance</p>
              <div className="flex items-center gap-3 text-xs font-mono mt-0.5">
                <span>Accuracy: <strong className="text-teal-600 dark:text-teal-400">{userScore.accuracy}%</strong></span>
        ...
        {/* 3. 7 Day Challenge Streak */}
        <div className="space-y-2 pt-1">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-foreground flex items-center gap-1">
              7 Day Challenge Streak <Flame className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
            </span>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {streakDays.map((s) => (
              <div key={s.day} className="space-y-1">
                ...
                >
                  {s.completed ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : s.isToday ? "•" : ""}
                </div>
              </div>
            ))}
          </div>
        ...
        {/* 4. Weekly Milestone / Reward Card */}
