...
import { SupportedLanguage, ExecutionResult } from "./IDECodeEditor";
import {
  Sparkles,
  Send,
  Wand2,
...
export default function AIMentorSidebar({
  ...
  return (
    <aside aria-label="AI Assistant Panel" className="h-full w-full flex flex-col bg-[#0E131F] text-[#E2E8F0] overflow-hidden select-none border-l border-[#1E2638]">
      {/* =========================================================================
          1. AI MENTOR HEADER WITH CLOSE (✕) BUTTON
         ========================================================================= */}
      <div className="h-12 px-4 border-b border-[#1E2638] bg-[#10141D] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-[#8B5CF6]/15 border border-[#8B5CF6]/30 flex items-center justify-center text-[#A78BFA] shadow-sm">
            <Bot className="w-4 h-4" />
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <h2 className="text-xs font-bold text-white tracking-wide">AI Mentor</h2>
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" title="Ready" />
            </div>
            <p className="text-[10px] text-[#94A3B8]">PlaceMentor Intelligent Assistant</p>
          </div>
        </div>
        ...
        <button
          ...
          className="w-7 h-7 rounded-lg flex items-center justify-center text-[#94A3B8] hover:text-white hover:bg-[#1E2638] border border-transparent hover:border-[#28354D] transition-all"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {/* Navigation Sub-Tabs */}
      <div className="px-4 border-b border-[#1E2638] bg-[#0E131F] flex items-center gap-6 shrink-0 text-xs font-semibold">
        <button
          type="button"
      ...
      {activeTab === "chat" ? (
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
            {/* Greeting Card */}
            <div className="p-3.5 rounded-xl border border-purple-500/20 bg-gradient-to-br from-purple-500/10 via-[#141923] to-[#141923] space-y-1.5 shadow-sm">
              <div className="font-bold text-white flex items-center gap-1.5 text-xs">
                <span>Hi! I'm your AI Mentor</span>
                <span>👋</span>
              </div>
              <p className="text-[#94A3B8] text-[11px] leading-relaxed">
                Need guidance for <strong className="text-[#E2E8F0]">{problem.title}</strong>? Ask questions, request hints, or review optimal approaches below.
              </p>
            </div>
            ...
            {messages.length === 0 && (
              <div className="space-y-2 pt-1">
                <span className="text-[11px] font-semibold text-[#94A3B8]">Suggested Quick Prompts:</span>

                <div className="space-y-1.5">
                  <button
                    type="button"
                    onClick={() => handleQuickPrompt("hint")}
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] hover:bg-[#1C2331] text-[#E2E8F0] hover:border-[#8B5CF6]/50 transition-all text-left group"
                  >
                    <Wand2 className="w-3.5 h-3.5 text-[#A78BFA] shrink-0 group-hover:scale-110 transition-transform" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] hover:bg-[#1C2331] text-[#E2E8F0] hover:border-[#8B5CF6]/50 transition-all text-left group"
                  >
                    <Lightbulb className="w-3.5 h-3.5 text-[#FBBF24] shrink-0 group-hover:scale-110 transition-transform" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] hover:bg-[#1C2331] text-[#E2E8F0] hover:border-[#8B5CF6]/50 transition-all text-left group"
                  >
                    <Cpu className="w-3.5 h-3.5 text-[#34D399] shrink-0 group-hover:scale-110 transition-transform" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] hover:bg-[#1C2331] text-[#E2E8F0] hover:border-[#8B5CF6]/50 transition-all text-left group"
                  >
                    <Clock className="w-3.5 h-3.5 text-[#60A5FA] shrink-0 group-hover:scale-110 transition-transform" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] hover:bg-[#1C2331] text-[#E2E8F0] hover:border-[#8B5CF6]/50 transition-all text-left group"
                  >
                    <HardDrive className="w-3.5 h-3.5 text-[#F472B6] shrink-0 group-hover:scale-110 transition-transform" />
            ...
            {messages.map((msg) => (
              ...
              >
                {msg.sender === "mentor" && (
                  <div className="w-6 h-6 rounded-full bg-[#8B5CF6]/20 border border-[#8B5CF6]/40 flex items-center justify-center shrink-0 mt-0.5 text-[#A78BFA]">
            ...
            {isThinking && (
              <div className="flex gap-2 items-center text-[#94A3B8] text-[11px] p-2">
          ...
          <form onSubmit={handleSendMessage} className="p-3 border-t border-[#1E2638] bg-[#10141D] shrink-0">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full pl-3.5 pr-10 py-2.5 text-xs rounded-xl border border-[#28354D] bg-[#141923] text-[#E2E8F0] placeholder-[#64748B] focus:outline-none focus:border-[#8B5CF6] transition-colors"
              />
              <button
                type="submit"
                disabled={!inputText.trim()}
                className="absolute right-2 p-1.5 text-[#8B5CF6] hover:text-[#A78BFA] disabled:opacity-40 transition-colors rounded-lg hover:bg-[#1E2638]"
                title="Send message"
              >
        ...
        /* Study Pathway Tab */
        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
          <div className="p-3.5 rounded-xl border border-[#1E2638] bg-[#141923] space-y-2">
            <div className="flex items-center gap-1.5 font-bold text-white">
              <BookOpen className="w-3.5 h-3.5 text-[#A78BFA]" />
              <span>Recommended Study Pathway</span>
            </div>
            <p className="text-[#94A3B8] text-[11px] leading-relaxed">
              Master core algorithms and data structures related to <strong className="text-[#E2E8F0]">{problem.topic}</strong>.
            </p>
          </div>

          <div className="space-y-2">
            <span className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Suggested Next Problems</span>
            <div className="space-y-1.5">
              {["Contains Duplicate", "Valid Anagram", "Group Anagrams", "Top K Frequent Elements"].map((title, i) => {
                ...
                return (
                  <div
                    ...
                    className="p-2.5 rounded-xl border border-[#1E2638] bg-[#141923] flex items-center justify-between hover:border-[#8B5CF6]/40 transition-all cursor-pointer group"
                    title={targetProb ? `Open problem: ${targetProb.title}` : undefined}
                  >
                    <span className="font-medium text-[#E2E8F0] group-hover:text-white">{title}</span>
                    <ArrowRight className="w-3.5 h-3.5 text-[#64748B] group-hover:text-[#A78BFA] group-hover:translate-x-0.5 transition-all" />
                  </div>
                );
              })}
            </div>
