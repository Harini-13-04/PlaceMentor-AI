...
import { Home, Code2, MessageSquare, Sparkles, User, Bell, Settings } from "lucide-react";
    ...
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/10 bg-card/30 backdrop-blur-3xl flex flex-col z-20 relative">
        <div className="p-6 flex items-center gap-3">
          ...
          <span className="font-display font-bold text-xl tracking-tight text-white">PlaceMentor</span>
        </div>
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/5 cursor-pointer transition-colors">
            <img src="https://i.pravatar.cc/150?u=12" alt="User" className="w-10 h-10 rounded-full border border-white/20" />
            <div>
              <p className="text-sm font-medium text-white">Alex Chen</p>
              <p className="text-xs text-primary font-medium mt-0.5">Level 12 • 4500 XP</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-background">
        {/* Topbar */}
        <header className="h-16 border-b border-white/10 bg-background/80 backdrop-blur-md flex items-center justify-between px-8 z-10 sticky top-0">
